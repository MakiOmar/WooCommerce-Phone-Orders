...
class WC_Phone_Orders_Loader {
	...
	public function __construct() {
        ...
        add_action( 'woocommerce_loaded', function(){
            if( version_compare( WC_VERSION, '7.1', '>=' ) && wc_get_container()->get( CustomOrdersTableController::class )->custom_orders_table_usage_is_enabled() ) {
		...
		add_filter( 'plugin_action_links_' . WC_PHONE_ORDERS_BASENAME, array( $this, 'add_action_links' ) );
                ...
                add_action( 'before_woocommerce_init', function() {
                    if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
                        \Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', WC_PHONE_ORDERS_PLUGIN_PATH_FILE, true );
                    }
                } );
	}
	...
	public static function check_user_capability() {
	    ...
	    add_action( 'manage_woocommerce_page_wc-orders_custom_column', array($this, 'manage_orders_custom_column'), 999, 2 );
	...
	public function add_billing_phone_email_to_wpo_customer_formatted_address() {
		add_filter( 'wpo_customer_formatted_address', function ( $fields, $customer_data, $type ) {
			if ( ( $type == 'billing' ) && isset( $customer_data['billing_phone'] ) ) {
				$fields['wpo_billing_phone'] = $customer_data['billing_phone'] ? sprintf('<a href="tel:%1$s">%1$s</a>', $customer_data['billing_phone']) : $customer_data['billing_phone'];
			}
			if ( ( $type == 'billing' ) && isset( $customer_data['billing_email'] ) ) {
				$fields['wpo_billing_email'] = $customer_data['billing_email'] ? sprintf('<a href="mailto:%1$s" target=_blank>%1$s</a>', $customer_data['billing_email']) : $customer_data['billing_email'];
			}
