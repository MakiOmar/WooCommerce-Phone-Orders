...
abstract class WC_Phone_Orders_Admin_Abstract_Page {
	...
	protected function get_customer_by_id( $customer_id ) {
		...
		$customer_data = array(
			...
			// Company-specific billing fields
			'billing_billing_company_checkbox' => isset( $customer['billing_billing_company_checkbox'] ) ? $customer['billing_billing_company_checkbox'][0] : '',
			'billing_billing_company_vat'      => isset( $customer['billing_billing_company_vat'] ) ? $customer['billing_billing_company_vat'][0] : '',
			'billing_short_address'            => isset( $customer['billing_short_address'] ) ? $customer['billing_short_address'][0] : '',
			'billing_building_number'          => isset( $customer['billing_building_number'] ) ? $customer['billing_building_number'][0] : '',
			'billing_address_second'           => isset( $customer['billing_address_second'] ) ? $customer['billing_address_second'][0] : '',
			'billing_district'                 => isset( $customer['billing_district'] ) ? $customer['billing_district'][0] : '',
			'billing_postal_code'              => isset( $customer['billing_postal_code'] ) ? $customer['billing_postal_code'][0] : '',
 ...
 $data['name'] = rawurldecode( $product->get_name() );
			if ( method_exists( $product, 'get_variation_attributes' ) ) {
				$data['attr'] =implode( " / ", $product->get_variation_attributes('pa_الحجم')) ;
			}
 
            
            $data['attr'] = isset( $data['attr'] ) ? urldecode($data['attr']) : '';

            $order = apply_filters( 'wpo_autocomplete_product_fields', array( 'name','attr', 'sku', 'price', 'status','qty' ) );
	...
	protected function get_updated_customer( $id, $customer_data, $request ) {
	    ...
	    if ( $customer_data instanceof WC_Data_Exception ) {
		    ...
		    // Hydrate company-specific billing fields from user meta for prefill.
		    ...
		    foreach ( $company_meta_keys as $meta_key ) {
			    $customer[ $meta_key ] = get_user_meta( $id, $meta_key, true );
		    }
