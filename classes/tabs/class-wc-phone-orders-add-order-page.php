	...
	public function render() {
		...
		$this->tab_data = array(
                    ...
                    'editAddressModalSettings'   => array(
                        ...
                        'addressFieldsOrder'		     => apply_filters('wpo_edit_address_modal_address_fields_order', array('country', 'address_1', 'billing_company_checkbox', 'address_2', 'city', 'state', 'postcode', 'billing_company_vat', 'short_address', 'building_number', 'address_second', 'district', 'postal_code')),
			...
			'productHistoryTitle'                   => __( 'أرشيف المنتجات', 'phone-orders-for-woocommerce' ),
			'cancelLabel'                           => __( 'Cancel', 'phone-orders-for-woocommerce' ),
			'addToCartLabel'                        => __( 'Add to cart', 'phone-orders-for-woocommerce' ),
	    ...
	    $fields = array(
            ...
            'billingAddress' => array(
	            ...
	            'fields' => array(
		            ...
		            'billing_company_checkbox' => array(
			            'label' => __( 'مؤسسة ؟', 'phone-orders-for-woocommerce' ),
			            'value' => '',
		            ),
		            'phone' => array(
		            ...
		            'billing_company_vat' => array(
			            'label' => __( 'الرقم الضريبي للمؤسسة', 'phone-orders-for-woocommerce' ),
			            'value' => '',
		            ),
		            'short_address' => array(
			            'label' => __( 'العنوان المختصر', 'phone-orders-for-woocommerce' ),
			            'value' => '',
		            ),
		            'building_number' => array(
			            'label' => __( 'رقم المبنى', 'phone-orders-for-woocommerce' ),
			            'value' => '',
		            ),
		            'address_second' => array(
			            'label' => __( 'الرقم الفرعي', 'phone-orders-for-woocommerce' ),
			            'value' => '',
		            ),
		            'district' => array(
			            'label' => __( 'الحي', 'phone-orders-for-woocommerce' ),
			            'value' => '',
		            ),
		            'postal_code' => array(
			            'label' => __( 'الرمز البريدي', 'phone-orders-for-woocommerce' ),
			            'value' => '',
		            ),
                )
            ),
...
//		$item_subtotal = ( isset( $item['line_subtotal'] ) ) ? esc_attr( wc_format_localized_price( $item['line_subtotal'] ) ) : '';
	...
	protected function create_customer( $request ) {
		...
		update_user_meta( $user_id, 'billing_first_name', $data['first_name'] . ' ' . $data['last_name'] );
		update_user_meta( $user_id, 'nickname', $username );
		update_user_meta( $user_id, 'billing_phone', $username );
		update_user_meta( $user_id, 'digits_phone', $data['phone'] );
		$countryCodes = [
			'AL' => '+355',
			'DZ' => '+213',
			'AW' => '+297',
		...
		$countryCode = $countryCodes[$data['country']];
		update_user_meta( $user_id, 'digt_countrycode', $countryCode );
		$tab = explode($countryCode, $data['phone']);
		update_user_meta( $user_id, 'digits_phone_no', $tab[1] );
		...
		foreach ( $billing_fields as $field ) {
			if ( $field != "phone" AND $field != "first_name" ) {
...
//			return $this->wpo_send_json_error( __( 'Recalculate cart changed cart items', 'phone-orders-for-woocommerce' ) );
	...
	protected function create_order( $data, $set_status = true ) {
		...
		// Keep billing_postal_code and billing_postcode in sync.
		if ( isset( $checkout_data['billing_postal_code'] ) && $checkout_data['billing_postal_code'] !== '' ) {
			$checkout_data['billing_postcode'] = $checkout_data['billing_postal_code'];
		} elseif ( empty( $checkout_data['billing_postal_code'] ) && ! empty( $checkout_data['billing_postcode'] ) ) {
			$checkout_data['billing_postal_code'] = $checkout_data['billing_postcode'];
		}
		...
		// Hook to verify and fix line_subtotal after order is created
		// Use high priority to run after other plugins
		add_action( 'woocommerce_checkout_order_processed', array( $this, 'action_woocommerce_checkout_order_processed_fix_subtotal' ), 999, 3 );
		// Also hook into wpo_order_created as a final safety net
		add_action( 'wpo_order_created', array( $this, 'action_wpo_order_created_fix_subtotal' ), 999, 2 );
		...
		// Don't remove the post-processing hook yet - it needs to run when woocommerce_checkout_order_processed fires
		...
		// Save company-specific billing fields to the order meta.
		$company_meta_keys = array(
			'billing_billing_company_checkbox',
		...
		foreach ( $company_meta_keys as $meta_key ) {
			if ( isset( $checkout_data[ $meta_key ] ) ) {
				$order->update_meta_data( $meta_key, $checkout_data[ $meta_key ] );
			}
		}
		$order->save();
		...
		// Now remove the post-processing hooks after they have run
		remove_action( 'woocommerce_checkout_order_processed', array( $this, 'action_woocommerce_checkout_order_processed_fix_subtotal' ), 999 );
		$order    = wc_get_order( $order_id );
		...
		// Remove the final safety net hook after it has run
		remove_action( 'wpo_order_created', array( $this, 'action_wpo_order_created_fix_subtotal' ), 999 );
    ...
    public function action_woocommerce_checkout_create_order_line_item($item, $cart_item_key, $values, $order) {
		...
		if ( isset($values['custom_meta_fields']) ) {
			// Prevent internal WooCommerce meta keys from being set via custom_meta_fields
			...
			foreach ($values['custom_meta_fields'] as $meta) {
				// Skip internal WooCommerce meta keys to prevent conflicts
...
//				}
		...
		if( $this->option_handler->get_option( "show_discount_amount_in_order" ) ) {
			try {
				...
				$calculated_subtotal = $original_price * (float) ( $values['quantity'] );
			...
			// Always calculate line_subtotal from cart item data to ensure quantity is included
			// The issue: $values['line_subtotal'] might be unit price instead of line total
			$product = $values['data'];
			$quantity = isset( $values['quantity'] ) ? (float) $values['quantity'] : 1;
			
			
			
			// Try to get unit price from various sources
			$unit_price = null;
			$unit_price_source = '';
			
			// 1. Check for wpo_item_cost in cart item meta (from our plugin)
			if ( isset( $values['wpo_item_cost'] ) && is_numeric( $values['wpo_item_cost'] ) ) {
				$unit_price = (float) $values['wpo_item_cost'];
				$unit_price_source = 'wpo_item_cost';
				
			}
			// 2. Check for item_cost in values
			elseif ( isset( $values['item_cost'] ) && is_numeric( $values['item_cost'] ) ) {
				$unit_price = (float) $values['item_cost'];
				$unit_price_source = 'item_cost';
				
			}
			// 3. Calculate unit price from line_subtotal and verify
			elseif ( isset( $values['line_subtotal'] ) && $quantity > 0 ) {
				$calculated_unit_price = (float) $values['line_subtotal'] / $quantity;
				$product_unit_price = (float) $product->get_price();
				
				
				
				// If calculated unit price is close to product price, line_subtotal was correct
				// Otherwise, line_subtotal is likely the unit price itself
				$price_difference_percent = abs( $calculated_unit_price - $product_unit_price ) / max( $product_unit_price, 0.01 );
				
				
				
				if ( $price_difference_percent < 0.2 ) { // Within 20% - likely correct
					// line_subtotal already includes quantity, calculate unit price from it
					$unit_price = $calculated_unit_price;
					$unit_price_source = 'line_subtotal_calculated (already had quantity)';
					
				} else {
					// line_subtotal is likely unit price, use it directly
					$unit_price = (float) $values['line_subtotal'];
					$unit_price_source = 'line_subtotal_direct (treating as unit price)';
			...
			// 4. Fallback to product price
				...
				$unit_price_source = 'product->get_price()';
				
			}
			
			// Calculate line_subtotal = unit_price * quantity
			if ( $order->get_prices_include_tax() && $unit_price === (float) $product->get_price() ) {
				// Get price excluding tax for line subtotal (WooCommerce stores line_subtotal excluding tax)
				add_filter( 'woocommerce_prices_include_tax', "__return_true", PHP_INT_MAX );
				$unit_price_excl_tax = wc_get_price_excluding_tax( $product, array(
					'qty' => 1,
					'price' => $unit_price
				) );
				$line_subtotal = $unit_price_excl_tax * $quantity;
				remove_filter( 'woocommerce_prices_include_tax', "__return_true", PHP_INT_MAX );
			...
			// Calculate line_total using the same unit_price to avoid rounding/discount issues
			// line_total should equal line_subtotal unless there's an actual discount
			// Check for discount via meta/coupons/mismatch with cart line_total
			$has_actual_discount = false;
			if ( isset( $values['wpo_item_discount'] ) && ! empty( $values['wpo_item_discount']['discount'] ) && (float) $values['wpo_item_discount']['discount'] != 0 ) {
				$has_actual_discount = true;
				
			}
			// Get line_total from values (from cart item data)
			$line_total_from_values = isset( $values['line_total'] ) ? (float) $values['line_total'] : null;
			
			// Also try to get it directly from cart if available and not in values
			if ( $line_total_from_values === null && function_exists( 'WC' ) && WC()->cart && isset( $cart_item_key ) ) {
				$cart_item = WC()->cart->get_cart_item( $cart_item_key );
				if ( $cart_item && isset( $cart_item['line_total'] ) ) {
					$line_total_from_values = (float) $cart_item['line_total'];
				}
			}
			
			// Check for coupons in cart - this is the most reliable indicator
			$cart_has_coupons = false;
			if ( function_exists( 'WC' ) && WC()->cart ) {
				$applied_coupons = WC()->cart->get_applied_coupons();
				$cart_has_coupons = ! empty( $applied_coupons ) && count( (array) $applied_coupons ) > 0;
			}
			
			// Always prefer cart's line_total when available - it's the source of truth from WooCommerce
			// WooCommerce has already calculated it with all discounts applied (item-level and cart-level)
			// This includes _wpo_cart_discount and any other coupons
			if ( $line_total_from_values !== null ) {
				// Always use cart's line_total - it includes all discount calculations
				$item->set_total( $line_total_from_values );
			} elseif ( $cart_has_coupons ) {
				// Coupons exist but no line_total available - this is unusual
				// In this case, we need to preserve the discount somehow
				// Check if line_subtotal from values differs from our calculation
				$line_subtotal_from_values = isset( $values['line_subtotal'] ) ? (float) $values['line_subtotal'] : null;
				if ( $line_subtotal_from_values !== null && abs( $line_subtotal_from_values - $line_subtotal ) > 0.001 ) {
					// Use cart's line_subtotal - it might already include discount calculations
					$item->set_total( $line_subtotal_from_values );
				} else {
					// Fallback to our calculated value
					$item->set_total( $line_subtotal );
				}
			} else {
				// No coupons, no line_total - use our calculated value
				$item->set_total( $line_subtotal );
			}
			
			// Verify what was actually set
			$set_subtotal = $item->get_subtotal();
			$set_total = $item->get_total();
	...
	/**
	 * Fix line_subtotal after order is processed in case it was overwritten
	 * 
	 * @param int $order_id
	 * @param array $posted_data
	 * @param WC_Order $order
	 */
	public function action_woocommerce_checkout_order_processed_fix_subtotal( $order_id, $posted_data, $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			// Get the actual meta value from database to see what's really stored
			global $wpdb;
			$meta_table = $wpdb->prefix . 'woocommerce_order_itemmeta';
			$meta_value = $wpdb->get_var( $wpdb->prepare(
				"SELECT meta_value FROM {$meta_table} WHERE order_item_id = %d AND meta_key = '_line_subtotal'",
				$item_id
			) );
			
			$db_subtotal = $meta_value ? (float) $meta_value : $current_subtotal;
			
			
			// Always check if subtotal needs fixing for quantity > 1
			// Use DB value if available, as get_subtotal() might return cached value
			if ( $quantity > 1 && ( $db_subtotal > 0 || $current_subtotal > 0 ) ) {
				// Check DB value first - if it's wrong, fix it
				$db_unit_price = $db_subtotal > 0 ? $db_subtotal / $quantity : 0;
				...
				if ( $product ) {
					...
					// Check if DB value looks wrong (unit price instead of line total)
					// CORRECT: If db_value / quantity ≈ product_price, then db_value is line total (correct)
					// WRONG: If db_value ≈ product_price (without dividing by quantity), then db_value is unit price (wrong)
					
					// Check if DB value itself is close to product price (within 5%)
					$db_as_unit_price_diff = abs( $db_subtotal - $product_price ) / max( $product_price, 0.01 );
					
					// Check if DB value divided by quantity is close to product price (should be true if correct)
					$db_unit_price_diff = $db_unit_price > 0 ? abs( $db_unit_price - $product_price ) / max( $product_price, 0.01 ) : 1;
					
					
					
					$needs_fix = false;
					
					// If DB value itself is very close to product price (< 5% difference),
					// then DB value is unit price (WRONG), not line total
					if ( $db_subtotal > 0 && $db_as_unit_price_diff < 0.05 && $db_unit_price_diff > 0.1 ) {
						// DB value ≈ product_price but DB value / quantity ≠ product_price
						// This means DB value is unit price, not line total - definitely wrong
						$needs_fix = true;
							
					} else {
						// Check using get_subtotal() method as backup
						$calculated_unit_price = $current_subtotal / $quantity;
						$price_diff_percent = abs( $calculated_unit_price - $product_price ) / max( $product_price, 0.01 );
						
						
						// If calculated unit price from get_subtotal() is not close to product price, might need fix
					...
					if ( $needs_fix ) {
						// The subtotal might be wrong - try to get wpo_item_cost from cart or calculate correctly
						$cart_item_meta = $item->get_meta( 'wpo_item_cost', true );
						
						if ( $cart_item_meta && is_numeric( $cart_item_meta ) ) {
							$unit_price = (float) $cart_item_meta;
							$correct_subtotal = $unit_price * $quantity;
							
						} elseif ( $order->get_prices_include_tax() ) {
							$unit_price_excl_tax = wc_get_price_excluding_tax( $product );
							$correct_subtotal = $unit_price_excl_tax * $quantity;
							
						} else {
							$correct_subtotal = $product_price * $quantity;
							
						}
						
						
						
						$item->set_subtotal( $correct_subtotal );
						$item->save();
						
						// Force update the meta directly in database to ensure it's saved
						// meta_value is stored as TEXT, so use %s format
						$updated_rows = $wpdb->update(
							$meta_table,
							array( 'meta_value' => (string) $correct_subtotal ),
							array( 
						...
						// Also ensure line_total matches line_subtotal (unless there's a discount)
						// Check if there's an actual discount on this item or order-level coupons/discounts
						$has_discount = $item->get_meta( $this->meta_key_order_item_discount, true );
						if ( empty( $has_discount ) ) {
							// Also check wpo_item_discount as fallback
							$has_discount = $item->get_meta( 'wpo_item_discount', true );
						}
						
						// Check for coupons/discounts - check both order and cart
						$order_has_discounts = ( (float) $order->get_discount_total() > 0 ) || ( ! empty( $order->get_coupon_codes() ) );
						$cart_has_coupons = false;
						if ( function_exists( 'WC' ) && WC()->cart ) {
							$applied_coupons = WC()->cart->get_applied_coupons();
							$cart_has_coupons = ! empty( $applied_coupons ) && count( (array) $applied_coupons ) > 0;
						...
						// Also check if current line_total differs from subtotal (indicates discount was applied)
						$current_line_total = (float) $item->get_total();
						$line_total_differs = abs( $current_line_total - $correct_subtotal ) > 0.001;
						
						// Only update line_total if there's no discount detected
						// If line_total already differs from subtotal, it means a discount was applied - preserve it
						if ( ( empty( $has_discount ) || empty( $has_discount['discount'] ) || (float) $has_discount['discount'] == 0 ) 
							&& ! $order_has_discounts 
							&& ! $cart_has_coupons
							&& ! $line_total_differs ) {
							// No discount - line_total should match line_subtotal
							$item->set_total( $correct_subtotal );
							$item->save();
							
							// Also update _line_total in DB directly
							$wpdb->update(
								$meta_table,
								array( 'meta_value' => (string) $correct_subtotal ),
								array( 
						...
						$verified_subtotal = $item->get_subtotal();
						$verified_total = $item->get_total();
						$verified_db_subtotal = $wpdb->get_var( $wpdb->prepare(
							"SELECT meta_value FROM {$meta_table} WHERE order_item_id = %d AND meta_key = '_line_subtotal'",
							$item_id
						) );
						$verified_db_total = $wpdb->get_var( $wpdb->prepare(
							"SELECT meta_value FROM {$meta_table} WHERE order_item_id = %d AND meta_key = '_line_total'",
							$item_id
						) );
		...
		// Calculate and set order discount_total from cart if coupons were applied
		// This ensures the order totals reflect the discount correctly
		if ( function_exists( 'WC' ) && WC()->cart ) {
			$applied_coupons = WC()->cart->get_applied_coupons();
			if ( ! empty( $applied_coupons ) ) {
				$cart_discount_total = WC()->cart->get_discount_total();
				$cart_discount_tax = WC()->cart->get_discount_tax();
				
				// Calculate discount from line items (sum of subtotal - total for all items)
				$calculated_discount = 0;
				foreach ( $order->get_items() as $item ) {
					$item_subtotal = (float) $item->get_subtotal();
					$item_total = (float) $item->get_total();
					$calculated_discount += ( $item_subtotal - $item_total );
				}
				
				// Use the larger of cart discount or calculated discount (to handle rounding differences)
				$discount_to_apply = max( $cart_discount_total, $calculated_discount );
				
				if ( $discount_to_apply > 0 ) {
					$order->set_discount_total( $discount_to_apply );
					$order->set_discount_tax( $cart_discount_tax );
					
					// Ensure coupons are added to the order
	...
	/**
	 * Final safety net - fix line_subtotal after wpo_order_created hook
	 * This runs after all other order processing hooks
	 * 
	 * @param WC_Order $order
	 * @param array $cart
	 */
	public function action_wpo_order_created_fix_subtotal( $order, $cart ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$db_subtotal = (float) $meta_value;
			$db_unit_price = $db_subtotal / $quantity;
			...
			// Check if DB value itself is close to product price (within 5%)
			// If so, DB value is unit price (WRONG), not line total
			$db_as_unit_price_diff = abs( $db_subtotal - $product_price ) / max( $product_price, 0.01 );
			$db_unit_price_diff = abs( $db_unit_price - $product_price ) / max( $product_price, 0.01 );
			
			
			
			// If DB value itself is very close to product price (< 5% difference),
			// AND DB value / quantity is NOT close to product price,
			// then DB value is unit price (WRONG), not line total
			$should_fix = ( $db_as_unit_price_diff < 0.05 && $db_unit_price_diff > 0.1 );
			
			
			if ( $should_fix ) {
				
				// Resolve unit price with highest priority to wpo_item_cost meta, then cart item meta, then product price
				$unit_price = null;
				$meta_unit_price = $item->get_meta( 'wpo_item_cost', true );
				if ( $meta_unit_price !== '' && $meta_unit_price !== null && is_numeric( $meta_unit_price ) ) {
					$unit_price = (float) $meta_unit_price;
				} elseif ( ! empty( $cart['items'] ) ) {
					foreach ( $cart['items'] as $cart_item ) {
						$cart_product_id = isset( $cart_item['variation_id'] ) && $cart_item['variation_id'] ? $cart_item['variation_id'] : ( isset( $cart_item['product_id'] ) ? $cart_item['product_id'] : 0 );
						if ( $cart_product_id == $product_id ) {
							if ( isset( $cart_item['wpo_item_cost'] ) && is_numeric( $cart_item['wpo_item_cost'] ) ) {
								$unit_price = (float) $cart_item['wpo_item_cost'];
								break;
							}
							if ( isset( $cart_item['item_cost'] ) && is_numeric( $cart_item['item_cost'] ) ) {
								$unit_price = (float) $cart_item['item_cost'];
								break;
							}
						}
					}
				...
				// Also check if current line_total differs from subtotal (indicates discount was applied)
				$current_line_total_db = $wpdb->get_var( $wpdb->prepare(
					"SELECT meta_value FROM {$meta_table} WHERE order_item_id = %d AND meta_key = '_line_total'",
					$item_id
				) );
				$current_line_total = $current_line_total_db ? (float) $current_line_total_db : (float) $item->get_total();
				$line_total_differs = abs( $current_line_total - $correct_subtotal ) > 0.001;
				
				$should_set_total = ( empty( $has_discount ) || empty( $has_discount['discount'] ) || (float) $has_discount['discount'] == 0 ) 
					&& ! $order_has_discounts 
					&& ! $cart_has_coupons
					&& ! $line_total_differs;
				...
				// Also update _line_total to match _line_subtotal if no discount
				if ( $should_set_total ) {
					$updated_total = $wpdb->update(
						$meta_table,
						array( 'meta_value' => (string) $correct_subtotal ),
						array( 
							'order_item_id' => $item_id,
				...
				// Verify the update
				$verified_subtotal = $wpdb->get_var( $wpdb->prepare(
					"SELECT meta_value FROM {$meta_table} WHERE order_item_id = %d AND meta_key = '_line_subtotal'",
					$item_id
				) );
				$verified_total = $wpdb->get_var( $wpdb->prepare(
					"SELECT meta_value FROM {$meta_table} WHERE order_item_id = %d AND meta_key = '_line_total'",
					$item_id
				) );
		...
		// Calculate and set order discount_total from line items if coupons were applied
		// This ensures the order totals reflect the discount correctly
		if ( function_exists( 'WC' ) && WC()->cart ) {
			$applied_coupons = WC()->cart->get_applied_coupons();
			if ( ! empty( $applied_coupons ) ) {
		...
		foreach ( $products as $product_id => $product) {
			...
			$result[] = apply_filters( 'wpo_search_products_result_item', array(
				...
				'add_to_exclude'	=> !$product->is_type( "grouped" ),
                                ...
                                'stock_multiplier'      => ( function( $product ) {
					$multiplier = (int) $product->get_meta( '_stock_multiplier' );
					if ( ! $multiplier && $product->is_type( 'variation' ) ) {
						$parent = wc_get_product( $product->get_parent_id() );
						if ( $parent ) {
							$multiplier = (int) $parent->get_meta( '_stock_multiplier' );
						}
					}
	...
	foreach ( $products_ids as $product_id) {
		...
		foreach ($ids as $id) {
		    ...
		    // qty step/min and stock for variations list
		    $default_qty_step = $this->option_handler->get_option( 'allow_to_input_fractional_qty' ) ? '0.01' : '1';
		    $qty_step = apply_filters( 'woocommerce_quantity_input_step', $default_qty_step, $product );
		    if ( ! is_numeric( $qty_step ) ) {
			$qty_step = $default_qty_step;
		    ...
		    // Compute in_stock with parent fallback for variations
		    $in_stock = null;
		    if ( $product->managing_stock() ) {
			$qty = $product->get_stock_quantity();
			if ( is_null( $qty ) && $product->is_type( 'variation' ) ) {
			    $parent = wc_get_product( $product->get_parent_id() );
			    if ( $parent && $parent->managing_stock() ) {
				$qty = $parent->get_stock_quantity();
				// backorders on parent?
				if ( $parent->backorders_allowed() ) {
				    $qty = null;
				}
			    }
			}
			if ( ! is_null( $qty ) ) {
			    $in_stock = $product->is_on_backorder( (int)$qty + 1 ) ? null : $qty;
			}
		    } else {
			// If not managing stock at variation level, attempt parent if variation
			if ( $product->is_type( 'variation' ) ) {
			    $parent = wc_get_product( $product->get_parent_id() );
			    if ( $parent && $parent->managing_stock() ) {
				$qty = $parent->get_stock_quantity();
				$in_stock = $parent->is_on_backorder( (int)$qty + 1 ) ? null : $qty;
			    }
			}
		    }
