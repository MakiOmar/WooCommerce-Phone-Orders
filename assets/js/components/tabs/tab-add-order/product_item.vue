...
<script>
    ...
    if (this.itemDiscountValue) {
      ...
      this.updateItem();
	    ...
	    maxQty () {
		// For variations, enforce qty * stock_multiplier <= in_stock
		const stock = typeof this.item.in_stock !== 'undefined' ? this.item.in_stock : null;
		if (stock === null || stock === '' || stock === false) {
			return null;
		}
		const isVariation = !!this.item.variation_id;
		const multiplier = isVariation ? (this.item.stock_multiplier ? parseInt(this.item.stock_multiplier) : 1) : 1;
		if (!multiplier || multiplier <= 1) {
			return stock;
		}
		return Math.floor(stock / multiplier);
	    },
        },
        methods: {
            updateItem () {
      ...
      discountData = {
		...
		this.$store.commit('add_order/setAdpRemoveFromCartGifts', removeAdpGifts);
            ...
            changeQty () {
                // Ensure we never commit invalid qty (which would trigger recalc)
                if (this.maxQty !== null && typeof this.maxQty !== 'undefined') {
                    const desired = +this.tmpQty;
                    const allowed = +this.maxQty;
                    if (desired > allowed) {
                        this.tmpQty = allowed;
                        this.qty = allowed;
                        return;
            ...
            onChangeQtyAndMaybeFormat (e) {
              ...
              let newQty = this.allowToInputFractionalQty ? parseFloat(qty).toFixed(digitsAfterDec) : qty;
              newQty = this.clampQtyToMax(newQty);
              this.tmpQty = newQty;
            },
