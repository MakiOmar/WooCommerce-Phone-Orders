		...
		<div class="wpo-search-options-block" :class="{'wpo-show-search-options': showProductSearchOptions}">
			...
			<div class="wpo-find-product-search-block">
			    ...
			    v-store-search-multiselect
                        ...
                        >
                            <template v-slot:noResult>
                                <span ref="productSearchNoResult">{{ showEmptyResultFindProducts ? noResultLabel : noOptionsTitle }}</span>
                            </template>
				...
				<label class="option__element__full-screen-mode">
					...
					<span class="option__input_price" v-if="isShowPriceInputInAdvancedSearch">
					    <input type="text" autocomplete="off" placeholder="0" v-model="multipleSelectedProducts[props.option.value]['item_cost']" size="4" v-bind:disabled="!cartEnabled" @mousedown="multipleSelectedProducts[props.option.value]['checked'] = true" class="price" @input="multipleSelectedProducts[props.option.value]['cost_updated_manually'] = true">
					</span>
				...
				<div class="find-results-full-screen-mode__search" @mousedown.stop @click.stop>
					...
					<input type="text" class="find-results-full-screen-mode__search__input" style="width: 100%" v-model="searchMultipleSelectedProducts" ref="searchMultipleSelectedProducts" :placeholder="findProductsSelectPlaceholder" id="find-results-full-screen-mode__search__input">
				    </div>
				</div>
			    </template>
				...
				{{ browseProductsMultipleSelectedProductsLabel }}
                            ...
                            <button id="wpo-product-history-button" class="btn btn-primary" v-if="cartEnabled && +customer.id" v-b-modal="'productHistoryCustomer'" :title="productHistoryButtonDescription">
                                {{ productHistoryButtonLabel }}
                            </button>
                            <button id="wpo-product-history-button" class="btn btn-primary disabled" v-else :title="productHistoryButtonDescription">
                                {{ productHistoryButtonLabel }}
                            </button>
                        </div>
			<div class="wpo-find-product-select-buttons" v-show="isAllowConfigureProduct && product && !useConfigureProductActionAsDefault">
				...
				<table cellpadding="0" cellspacing="0" class="woocommerce_order_items">
                    <draggable tag="tbody" @end="changeProductItemsOrder" handle=".handle" :disabled="!cartEnabled" v-model="productList" item-key="key">
		    ...
		    <div class="col-12 col-md-7">
                    ...
                    <div class="col-12 col-md-5">
                        <table class="wc-order-totals">
                            ...
                            <tr v-if="showCheckboxDontApplyPricingRules">
                                <td>
                                    <input type="checkbox" class="option" v-model="dontApplyPricingRules" id="dont-apply-pricing-rules" :disabled="!cartEnabled">
                                </td>
                                <td colspan=3>
			    ...
			    <tr v-if="giftCardEnabled" class="gift-card-list-item" v-for="(card, index) in giftCardList">
                                <td class="label-total">{{ giftCardLabel }}: <div v-html="card.title"></div></td>
                                <td width="1%"></td>
                                <td>
	    ...
	    addLabel: {
          ...
          useDefaultQtyInAdvancedSearch: {
            ...
            dontApplyPricingRulesLabel: {
                default: function() {
                  return 'Don\'t apply pricing rules to this order';
                }
            },
            pricingRulesEnabled: {
                default: function() {
...
//                    });
			...
			return;
                    ...
                    this.asyncFind(this.$refs.productSelectSearch.search, this.isExistsAdditionalProductSearchParams);
		...
		if (cart) {
            ...
            this.selectedItems = this.selectedItems.filter((item) => {
                return cart.deleted_items.map((_item) => { return +_item.variation_id || +_item.id; }).indexOf(+item) === -1;
            });
