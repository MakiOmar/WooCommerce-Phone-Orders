...
//            }
	...
	public function update_customer( $id, $customer_data ) {
		...
		if ( isset( $customer_data['billing_postal_code'] ) && $customer_data['billing_postal_code'] !== '' ) {
			$customer_data['billing_postcode'] = $customer_data['billing_postal_code'];
		} elseif ( empty( $customer_data['billing_postal_code'] ) && ! empty( $customer_data['billing_postcode'] ) ) {
			$customer_data['billing_postal_code'] = $customer_data['billing_postcode'];
		}
		...
		// Persist custom company fields to user meta.
		if ( $id ) {
			...
			foreach ( $company_meta_keys as $meta_key ) {
				if ( isset( $customer_data[ $meta_key ] ) ) {
					update_user_meta( $id, $meta_key, $customer_data[ $meta_key ] );
				}
			}
			// Also mirror billing_postal_code to the standard billing_postcode meta for completeness.
			if ( isset( $customer_data['billing_postal_code'] ) ) {
				update_user_meta( $id, 'billing_postcode', $customer_data['billing_postal_code'] );
			}
		}
	...
	public function get_item_by_product( $product, array $item_data = array() ) {
        ...
        if( !$product->managing_stock() ) { //we know only status for the product
            ...
            // Attempt parent fallback if variation and parent manages stock
            if ( is_null( $in_stock ) && $product->is_type( 'variation' ) ) {
                $parent = wc_get_product( $product->get_parent_id() );
                if ( $parent && $parent->managing_stock() ) {
                    $qty_parent = $parent->get_stock_quantity();
                    $in_stock = $parent->backorders_allowed() ? null : $qty_parent;
                }
            }
        } else { // manage stock qty
            if(isset($item_data['reduced_stock'])) { //edit existing order ?
                $qty_current = $product->get_stock_quantity();
                // If variation qty is null, try parent qty
                if ( is_null( $qty_current ) && $product->is_type( 'variation' ) ) {
                    $parent = wc_get_product( $product->get_parent_id() );
                    if ( $parent && $parent->managing_stock() ) {
                        $qty_current = $parent->get_stock_quantity();
                    }
                }
                $in_stock = $qty_current > 0 ? $qty_current + $item_data['reduced_stock'] : $item_data['reduced_stock'];
            } else {
                $qty_current = $product->get_stock_quantity();
                if ( is_null( $qty_current ) && $product->is_type( 'variation' ) ) {
                    $parent = wc_get_product( $product->get_parent_id() );
                ...
                if ( ! isset( $in_stock ) ) {
                    $in_stock = $product->backorders_allowed() ? null : $qty_current;
                }
            }
        }
